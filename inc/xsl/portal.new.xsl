...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="topbar">
    	<xsl:apply-templates select="navigation-tree" mode="sidebar"/>
	</xsl:template>
	...
	<xsl:template match="dir | file | link" mode="topbar"> <!-- directory-structure -->
	...
	<xsl:template match="directory-structure" mode="topbar">  <!-- -->
        <xsl:value-of select="name()" />
	...
	<xsl:template match="/model/navigation-tree" mode="topbar">

                    <xsl:apply-templates select="dir | file" mode="">
						<xsl:sort select="name()"/>
					</xsl:apply-templates>
